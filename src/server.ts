...
export function buildServer(config: Config, database: FeatherDatabase) {
  ...
  app.post("/v1/search", async (request, reply) => {
    const parsed = z.object({ query: z.string().min(1).max(500), limit: z.number().int().min(1).optional() }).safeParse(request.body);
    if (!parsed.success) return reply.code(400).send({ status: "invalid_request", error: parsed.error.issues });
    try {
      const results = search(database, config, parsed.data.query, parsed.data.limit);
