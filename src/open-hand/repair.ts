...
const nativeSuppressionSelectors = new Set(["source_file_id", "relative_path", "section_id", "entity_id"]);
...
export const repairActionSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    action: z.literal("plan"), intent: z.enum(intents), storage_system: z.enum(storageSystems),
    selector_type: z.enum(selectorTypes), selector_value: z.string().min(1).max(2_000),
    correction_text: z.string().min(1).max(8_000).optional(),
    note: z.string().max(4_000).optional(), idempotency_key: z.string().min(1).max(500).optional(),
    ...sourceFields,
  }).strict(),
  z.object({ action: z.literal("apply"), repair_id: z.string().uuid(), confirm_intent: z.enum(intents) }).strict(),
  z.object({ action: z.literal("retract"), repair_id: z.string().uuid(), note: z.string().max(4_000).optional() }).strict(),
]);
...
const capabilityRegistry: Record<StorageSystem, {
  ...
  provider: {
    authority: "External model/provider infrastructure.", modes: {},
    limits: ["No verified deletion adapter is available from Feather-Light."],
  },
  backup: {
    authority: "Host or external backup infrastructure.", modes: {},
    limits: ["No verified deletion adapter is available from Feather-Light."],
  },
  canonical_archive: {
...
export function repairCapabilities(): object {
  return {
    ...
    systems: Object.entries(capabilityRegistry).map(([storageSystem, value]) => ({
      storageSystem, authority: value.authority,
...
function row(database: FeatherDatabase, repairId: string) {
  return database.prepare(`
    SELECT repair_id AS repairId, requested_at AS requestedAt, intent,
      ...
      status, applied_at AS appliedAt, retracted_at AS retractedAt, result_json AS resultJson
    FROM open_hand_repairs WHERE repair_id=?
  `).get(repairId) as Record<string, unknown> | undefined;
}

function targetExists(database: FeatherDatabase, selectorType: string, selectorValue: string): boolean {
  if (selectorType === "source_file_id") return Boolean(database.prepare("SELECT 1 FROM source_files WHERE source_file_id=? AND deleted=0").get(selectorValue));
  if (selectorType === "relative_path") return Boolean(database.prepare("SELECT 1 FROM source_files WHERE relative_path=? AND deleted=0").get(selectorValue));
  if (selectorType === "section_id") return Boolean(database.prepare("SELECT 1 FROM source_sections s JOIN source_files f ON f.source_file_id=s.source_file_id WHERE s.section_id=? AND f.deleted=0").get(selectorValue));
  if (selectorType === "entity_id") return Boolean(database.prepare("SELECT 1 FROM entities WHERE entity_id=? AND retired=0").get(selectorValue));
  return false;
}
...
function plan(database: FeatherDatabase, action: Extract<RepairAction, { action: "plan" }>) {
  ...
  let mode = capabilityMode(action.storage_system, action.intent);
  if (mode === "native" && action.intent === "suppress_retrieval" && !nativeSuppressionSelectors.has(action.selector_type)) {
    mode = "unsupported";
  }
  const status = mode === "native" ? "pending" : mode === "unsupported" ? "rejected" : "external_required";
  if (action.idempotency_key) {
    const existing = database.prepare("SELECT repair_id AS repairId FROM open_hand_repairs WHERE idempotency_key=?").get(action.idempotency_key) as { repairId: string } | undefined;
    if (existing) return { created: false, repair: row(database, existing.repairId) };
  }
  const repairId = randomUUID();
  database.prepare(`
    INSERT INTO open_hand_repairs(
      repair_id, requested_at, intent, storage_system, selector_type, selector_value,
      correction_text, note, source_type, source_id, idempotency_key, capability_mode, status
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(repairId, new Date().toISOString(), action.intent, action.storage_system, action.selector_type,
    action.selector_value, action.correction_text ?? null, action.note ?? null, action.source_type,
    action.source_id, action.idempotency_key ?? null, mode, status);
...
function apply(database: FeatherDatabase, action: Extract<RepairAction, { action: "apply" }>) {
  return database.transaction(() => {
    ...
    if (repair.capabilityMode !== "native" || repair.storageSystem !== "feather_light_index" || repair.intent !== "suppress_retrieval") throw new Error("repair has no native apply adapter");
    const selectorType = String(repair.selectorType);
    const selectorValue = String(repair.selectorValue);
    if (!nativeSuppressionSelectors.has(selectorType)) throw new Error("selector is not supported for index suppression");
    if (!targetExists(database, selectorType, selectorValue)) throw new Error("suppression target not found");
    const suppressionId = randomUUID();
    const now = new Date().toISOString();
    database.prepare(`
      INSERT INTO retrieval_suppressions(suppression_id, repair_id, selector_type, selector_value, status, created_at)
      VALUES (?, ?, ?, ?, 'active', ?)
...
function retract(database: FeatherDatabase, action: Extract<RepairAction, { action: "retract" }>) {
  return database.transaction(() => {
    ...
    if (repair.status === "applied") {
      database.prepare("UPDATE retrieval_suppressions SET status='retracted', retracted_at=? WHERE repair_id=? AND status='active'").run(now, action.repair_id);
    }
    if (action.note) {
      database.prepare("UPDATE open_hand_repairs SET note=CASE WHEN note IS NULL OR note='' THEN ? ELSE note || char(10) || ? END WHERE repair_id=?").run(action.note, action.note, action.repair_id);
    }
    database.prepare("UPDATE open_hand_repairs SET status='retracted', retracted_at=? WHERE repair_id=?").run(now, action.repair_id);
...
function state(database: FeatherDatabase, action: Extract<RepairAction, { action: "state" }>) {
  ...
  const repairs = database.prepare(`
    SELECT repair_id AS repairId, requested_at AS requestedAt, intent,
      storage_system AS storageSystem, selector_type AS selectorType, selector_value AS selectorValue,
      correction_text AS correctionText, note, source_type AS sourceType, source_id AS sourceId,
      capability_mode AS capabilityMode, status, applied_at AS appliedAt,
      retracted_at AS retractedAt, result_json AS resultJson
