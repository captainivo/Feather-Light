...
describe("Open Hand repair API", () => {
  it("reports capabilities and records external plans without claiming application", async () => {
    ...
    const capabilities = await app.inject({
      method: "POST", url: "/v1/query",
      payload: { operation: "open_hand_repair", repair: { action: "capabilities" } },
    });
    expect(capabilities.statusCode).toBe(200);
    expect(capabilities.json()).toMatchObject({
      status: "ok", result: { automatic_deletion: false, intent_separation_required: true },
    });
    ...
    expect(plan.json()).toMatchObject({
      status: "ok",
      result: { created: true, repair: { status: "external_required", capabilityMode: "external_adapter" } },
    });
  });
  ...
  it("rejects an apply request whose exact confirmation is absent", async () => {
    ...
    const response = await app.inject({
      method: "POST", url: "/v1/query",
      payload: { operation: "open_hand_repair", repair: { action: "apply", repair_id: crypto.randomUUID() } },
    });
